...
def valid_session(handler: BaseHTTPRequestHandler) -> bool:
...
async function load(){var r=await fetch('/api/dashboard');if(r.status===401){location='/login';return}var d=await r.json(),m=d.missions||[],w=d.wins||[];var html="<div class='grid'><div class='card'><div class='muted'>Missions</div><h2>"+m.length+"</h2></div><div class='card'><div class='muted'>Proven wins</div><h2>"+w.length+"</h2></div><div class='card'><div class='muted'>Active</div><h2>"+m.filter(function(x){return x.status==='ACTIVE'}).length+"</h2></div></div>";html+="<div class='card'><h2>Career Mission Control</h2><form onsubmit='createMission(event)'><input id='objective' placeholder='Objective / target win' required><button class='btn'>Create Mission</button></form></div>";m.forEach(function(x){html+="<div class='card'><span class='pill'>"+esc(x.stage)+"</span> <span class='pill'>"+esc(x.status)+"</span><h3>"+esc(x.objective)+"</h3><div class='muted'>Proof captured: "+((x.evidence||[]).length)+" · Result: "+(Object.keys(x.result||{}).length?"yes":"pending")+"</div></div>"});html+="<div class='card'><h2>Win Ledger</h2>";if(!w.length)html+="<p class='muted'>No proven wins yet.</p>";w.forEach(function(x){html+="<div class='card win'><b>"+esc(x.target)+"</b><div class='muted'>Proof: "+(x.proof||[]).map(esc).join(" · ")+"</div><div>Next win: "+esc(x.next_win)+"</div></div>"});html+="</div>";document.getElementById('app').innerHTML=html}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=="/health": return self.json(200,{"status":"healthy","service":"c6-career-command-centre"})
        if path=="/login": return self.page(LOGIN)
        if not valid_session(self): return self.json(401,{"error":"authentication_required"}) if path.startswith("/api/") else self.page(LOGIN,401)
        if path=="/": return self.page(PAGE)
        if path=="/api/dashboard": return self.json(200,CareerStore().dashboard())
        if path=="/api/missions": return self.json(200,{"missions":CareerService().missions()})
    ...
    def do_POST(self):
        ...
        if path=="/login":
            user=data.get("username",[""])[0];password=data.get("password",[""])[0]
            ...
            return self.page(LOGIN,401)
        if not valid_session(self): return self.json(401,{"error":"authentication_required"})
        raw=self.rfile.read(0)
        if path=="/api/missions":
            return self.json(400,{"error":"use JSON body"}) 
        return self.json(404,{"error":"not_found"})
